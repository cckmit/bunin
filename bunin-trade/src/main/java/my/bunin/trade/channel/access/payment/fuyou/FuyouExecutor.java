...
import my.bunin.trade.channel.access.bean.PaymentQueryRequest;
import my.bunin.trade.channel.access.bean.PaymentQueryResponse;
...
import my.bunin.trade.channel.access.payment.fuyou.bean.PaymentQueryRequestMapper;
import my.bunin.trade.channel.access.payment.fuyou.bean.PaymentQueryResponseMapper;
import my.bunin.trade.channel.access.payment.fuyou.bean.RechargePaymentRequestMapper;
import my.bunin.trade.channel.access.payment.fuyou.bean.RechargePaymentResponseMapper;
import my.bunin.trade.channel.access.payment.fuyou.bean.TransMapper;
...
public class FuyouExecutor extends AbstractExecutor {
  ...
  /**
   * fuyou executor constructor.
   */
  public FuyouExecutor(HttpClient client) {
    this.client = client;

    stream = new XStream();
    stream.processAnnotations(RechargePaymentRequestMapper.class);
    stream.processAnnotations(RechargePaymentResponseMapper.class);
    stream.processAnnotations(PaymentQueryRequestMapper.class);
    stream.processAnnotations(PaymentQueryResponseMapper.class);
  ...
  protected String format(PaymentRequest request) {
    ...
    throw new UnsupportedOperationException(
        String.format("%s transaction type was unsupported. payment type: %s",
  ...
  private String formatRechargePaymentRequest(PaymentRequest request) {
    ...
    requestMapper.setSettleDate(transaction.getSettlementDate()
    ...
    String xml = stream.toXML(requestMapper);
  ...
  private String formatRechargePaymentQueryRequest(PaymentQueryRequest request) {
    ...
    requestMapper.setOrderNo(transaction.getSerialNo());
    requestMapper.setStartDate(LocalDateTime.now().minusDays(14)
        .format(DateTimeFormatter.BASIC_ISO_DATE));
  ...
  private String buildRequestContent(Request request, String xml, String type) {
    ...
    String plainMac = Joiner.on(StringUtils.VERTICAL).skipNulls()
        .join(configuration.getMerchantNo(), configuration.getPassword(),
  ...
  protected PaymentResponse parse(PaymentRequest request, String responseContent) {
    ...
    RechargePaymentResponseMapper responseMapper =
        (RechargePaymentResponseMapper) stream.fromXML(responseContent);
  ...
  protected PaymentQueryResponse parse(PaymentQueryRequest request, String responseContent) {
    PaymentQueryResponseMapper responseMapper =
        (PaymentQueryResponseMapper) stream.fromXML(responseContent);

    final List<TransMapper> transMappers = responseMapper.getTransactions();
